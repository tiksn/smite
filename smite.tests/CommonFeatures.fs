    ...
    testList "CommonFeatures"
        [ testCase "No common segment"
          <| fun _ ->
              ...
              Expect.equal (nsd |> Seq.length) (subject |> Seq.length)
                  "Count must be equal"
              let nsArray =
                  nsd
              ...
              for i = 0 to itemCount - 1 do
                  for j = 0 to nsArray.[i].Length - 1 do
                      if i <> j then
                          Expect.equal nsArray.[i].[j] fsArray.[i].[j]
                              "Item values must be equal"
          ...
          <| fun _ ->
              ...
              for i = 0 to itemCount - 1 do
                  for j = 0 to nsArray.[i].Length - 2 do
                      if i <> j then
                          Expect.equal nsArray.[i].[j + 1] fsArray.[i].[j]
                              "Item values must be equal"
          ...
          <| fun _ ->
              ...
              for i = 0 to itemCount - 1 do
                  for j = 0 to nsArray.[i].Length - 3 do
                      if i <> j then
                          Expect.equal nsArray.[i].[j + 2] fsArray.[i].[j]
                              "Item values must be equal" ]
